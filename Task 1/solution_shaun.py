...
def label_mask(array, mask):
    ...
    for y, x, val in zip(y_coords, x_coords, values):
        adjacent_label = label_mask[max([0, y - 1]):min([y + 2, array.shape[1]]),
                         max([0, x - 1]):min([x + 2, array.shape[1]])].max()
        if adjacent_label > 0:
    ...
    centroid = [((y_grid[f] * array[f]).sum() / array[f].sum(), (x_grid[f] * array[f]).sum() / array[f].sum()) for f in
                filters]
